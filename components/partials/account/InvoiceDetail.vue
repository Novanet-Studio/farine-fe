          ...
          <div class="row">
            <div class="col-md-4 col-12">
              <figure class="ps-block--invoice">
                ...
                <div class="ps-block__content">
                  <strong>
                    {{ invoice.attributes.fullName || "Desconocido" }}
                  </strong>
                  <p v-if="invoice.attributes.shippingAddress">
                    Dirección:
                    {{ invoice.attributes.shippingAddress.addressLine1 }},
                    {{ invoice.attributes.shippingAddress.locality }},
                    {{ invoice.attributes.shippingAddress.country }}
                    <br />
                    {{ invoice.attributes.shippingAddress.postalCode }},
                    <b>Apt, Casa: </b>
                    {{ invoice.attributes.shippingAddress.home }}
                  </p>
                </div>
              </figure>
            </div>
            ...
            <div class="col-md-5 col-12">
              <figure class="ps-block--invoice">
                ...
                <div
                  v-if="invoice.attributes.cardKind !== 'no aplica'"
                  class="ps-block__content"
                >
                  <p>
                    Pago: {{ invoice.attributes.cardKind }}
                    {{ invoice.attributes.cardType }},
                  </p>
                  <p>
                    Ultimos Cuatro digitos: {{ invoice.attributes.cardLast }}
                  </p>
                </div>
                <div v-else class="ps-block__content">
                  <p>Pago: {{ invoice.attributes.payment_method }}.</p>
                  <p>
                    Confirmacion de pago:
                    {{ invoice.attributes.payment_info[0].confirmacion }}
                  </p>
                </div>
              </figure>
            </div>
